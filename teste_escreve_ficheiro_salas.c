...
void insere_sala(char *sala, Next_exame exame) {
...
void escreve_ficheiro_salas(char *sala, Next_exame lista_exames, Next_exame exame) {
  Next_exame l_exames = lista_exames;
  Next_sala l_salas;
  FILE *fp = fopen("ficheiro_salas.txt", "a");
  char inutil[100], id[100], c;
  int count=0, tag=0, posicao;

  fscanf(fp, " %[^,]", id);
  while (tag == 0) {
    if (atoi(id) == l_exames->id) {
      tag=1;
      c = getc(fp);
      while (c != '\n') {
        posicao = ftell(fp);
...
int main() {
  ...
  Next_exame lista_exames, l_exames;
  ...
  printf("Em qual exame é que quer adicionar uma sala?\n");
  ...
  printf("Nova sala: ");
  scanf("%s", sala);
  insere_sala(sala, l_exames);
  escreve_ficheiro_salas(sala, lista_exames, l_exames);
}
